...
const getMument = async (mumentId: string, userId: string): Promise<MumentResponseDto | null | number> => {
    ...
    try {
        ...
        const user = await userDB.userData(mument.user_id.toString());
        if (user.length === 0) return constant.NO_MUMENT;
        ...
        const data: MumentResponseDto = {
            user: {
                _id: user[0].id,
                image: user[0].image as string,
                name: user[0].profile_id as string,
            },
