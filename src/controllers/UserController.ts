...
const deleteUserAndRevokeSocial = async (req: Request, res: Response) => {
    ...
    try {
        const data = await UserService.deleteUserAndRevokeSocial(userId, socialToken);
        ...
        switch (data) {
            ...
            case constant.KAKAO_UNLINK_FAIL:
                return res.status(statusCode.FORBIDDEN).send(util.fail(statusCode.BAD_REQUEST, message.KAKAO_UNLINK_FAIL));
